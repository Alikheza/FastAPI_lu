...
def create_course_API(course_validate:COR.Course_Info_In, db : Session=Depends(get_db)):
    if C_crud.read_course(db=db,id=course_validate.course_id) != None : raise HTTPException(status_code=404,detail='کد درس نمیتواند تکراری باشد')
    return C_crud.create_course(data=course_validate , db=db)


@router.get("/Read_course/{course_id}", response_model=COR.Course_Info_Out)
